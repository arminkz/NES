...
void InputManager::pollJoystick()
{
    ...
    //Left
    joystick1Directions[0] = joystick1Axes[0] < -0.5;
    
    //Right
    joystick1Directions[1] = joystick1Axes[0] > 0.5;

    //Up
    joystick1Directions[2] = joystick1Axes[1] < -0.5;

    //Down
    joystick1Directions[3] = joystick1Axes[1] > 0.5;
    ...
    for (int i = 0; i < joystick2ButtonCount; ++i) {
        if (joystick2Buttons[i] == GLFW_PRESS) {
            spdlog::info("Button {}: {}", i, joystick2Buttons[i]);
        }
    }
    ...
    //Left
    joystick2Directions[0] = joystick2Axes[0] < -0.5;

    //Right
    joystick2Directions[1] = joystick2Axes[0] > 0.5;

    //Up
    joystick2Directions[2] = joystick2Axes[1] < -0.5;

    //Down
    joystick2Directions[3] = joystick2Axes[1] > 0.5;
}