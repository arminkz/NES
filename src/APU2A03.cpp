...
{
    // APU is a fire and forget system, so we don't really read from it
    uint8_t data = 0x00;
    return data;
}
...
void APU2A03::cpuWrite(uint16_t addr, uint8_t data)
{
    ...
    case 0x4000:
        ...
        switch ((data & 0xC0) >> 6)
        {
        case 0x00: pulse1_seq.new_sequence = 0b01000000; pulse1_osc.duty_cycle = 0.125; break;
        case 0x01: pulse1_seq.new_sequence = 0b01100000; pulse1_osc.duty_cycle = 0.250; break;
        case 0x02: pulse1_seq.new_sequence = 0b01111000; pulse1_osc.duty_cycle = 0.500; break;
        case 0x03: pulse1_seq.new_sequence = 0b10011111; pulse1_osc.duty_cycle = 0.750; break;
        }
    ...
    case 0x4004:
        // Set pulse 2 duty cycle
        switch ((data & 0xC0) >> 6)
		{
		case 0x00: pulse2_seq.new_sequence = 0b01000000; pulse2_osc.duty_cycle = 0.125; break;
		case 0x01: pulse2_seq.new_sequence = 0b01100000; pulse2_osc.duty_cycle = 0.250; break;
		case 0x02: pulse2_seq.new_sequence = 0b01111000; pulse2_osc.duty_cycle = 0.500; break;
		case 0x03: pulse2_seq.new_sequence = 0b10011111; pulse2_osc.duty_cycle = 0.750; break;
		}
    ...
    case 0x4006:
        // Set pulse 2 frequency by setting the upper 8 bits of the reload value
        pulse2_seq.reload = (pulse2_seq.reload & 0xFF00) | data;
        break;

    case 0x4007:
        // Set pulse 2 frequency by setting the lower 3 bits of the reload value
...
void APU2A03::clock()
{
    ...
    if (apuClockCounter % 6 == 0)
    {
        ...
        pulse1_osc.frequency = 1789773.0 / (16.0 * (double)(pulse1_seq.reload + 1));  //nesdev.com wiki
        ...
        pulse2_osc.frequency = 1789773.0 / (16.0 * (double)(pulse2_seq.reload + 1));  //nesdev.com wiki
